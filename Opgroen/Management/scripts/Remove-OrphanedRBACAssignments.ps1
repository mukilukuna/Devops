param(
    [string]$ManagementGroupName = "mg-CustomerMg"
)
...
ForEach ($ScopeLocation in $AzResourceTree_Scope) {
    ...
    # Also checking if object is inherited from a parent location. Should only be object located on actual ScopeLocation
    if ($ScopeLocation) {
...
ForEach ($Entry in $Orphaned_Accounts_Array) {
    Write-Host "##[group] Orphaned RBAC Entry: #$i"
    ...
    try {
        $Uri = "$($Entry.Scope)/providers/Microsoft.Authorization/roleAssignments?api-version=2018-01-01-preview"
        $IAMviaAPI = Invoke-AzRestMethod -Path $Uri -Method GET
        $IAMviaAPIContent = $IAMviaAPI.Content | ConvertFrom-Json
        $apiEntry = $IAMviaAPIContent.value.properties | Where-Object { $_.principalId -eq $Entry.ObjectId }

        if ($apiEntry.principalType -notlike 'Foreign*') {
            $apiEntry

            $lock = Get-AzResourceLock -LockName 'iac-CanNotDelete' -Scope $Entry.Scope -ErrorAction SilentlyContinue
            if ($lock) {
                $lock | Remove-AzResourceLock -Force
            }
            $Entry | Remove-AzRoleAssignment
            if ($lock) {
                $lock | New-AzResourceLock -LockLevel CanNotDelete -Scope $Entry.Scope -Force
            }
        } else {
            Write-Output "Object is foreign principal"
        }
        Write-Output ""
    } catch {

        if ($_.Exception.Message -like "*PreconditionFailed*" ) {
            Write-Host "##[warning]We cannot remove the last owner from the scope. Please validate the owners on the scope."
        } else {
