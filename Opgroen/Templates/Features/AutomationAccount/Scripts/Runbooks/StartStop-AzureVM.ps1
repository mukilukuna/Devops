...
function AssertVirtualMachinePowerState
{
	...
	if($DesiredState -eq "Started" -and $currentStatus -notmatch "running")
	{
        ...
        else
        {
            ...
            $resourceManagerVM | Start-AzureRmVM | Out-Null
        }
	}
	...
	elseif($DesiredState -eq "StoppedDeallocated" -and $currentStatus -ne "deallocated")
	{
        ...
        else
        {
            ...
            $resourceManagerVM | Stop-AzureRmVM -Force | Out-Null
        }
	}
...
try
{
    ...
    Write-Output "Getting list of Azure resources of the virtualMachine type"
    $resourceManagerVMList = @(Get-AzureRmResource | Where-Object {$_.ResourceType -like "Microsoft.*/virtualMachines"} | Sort-Object Name)
    Write-Output "Found $($resourceManagerVMList.Count) virtual machines"

    # Get resource groups that are tagged for automatic shutdown of resources
    Write-Output "Finding resource groups with an AutoShutdownSchedule tag"
    ...
    foreach($vm in $resourceManagerVMList)
    {
        ...
        elseif($taggedResourceGroupNames -contains $vm.ResourceGroupName)
        {
            ...
            $schedule = ($parentGroup.Tags | Where-Object Name -eq "AutoShutdownSchedule")["Value"]
