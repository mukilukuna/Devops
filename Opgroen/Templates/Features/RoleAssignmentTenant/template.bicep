...
resource roleAssignments 'Microsoft.Authorization/roleAssignments@2022-04-01' = [for item in permissions: {
  name: guid(tenant().tenantId, item.name, item.roleDefinitionId)
  properties: {
...
output resourceName string = roleAssignments[0].name
@description('The resource ID of the Azure resource')
output resourceID string = roleAssignments[0].id
