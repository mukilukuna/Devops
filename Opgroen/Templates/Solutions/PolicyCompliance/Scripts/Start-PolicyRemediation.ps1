...
switch ($Type) {
    ...
    managementgroup {
        $nonCompliantPolicies = Get-AzPolicyState -ManagementGroupName $Id | Where-Object { $_.ComplianceState -eq "NonCompliant" -and ($_.PolicyDefinitionAction -eq "modify" -or $_.PolicyDefinitionAction -eq "deployIfNotExists" -or $_.PolicyDefinitionAciont -eq "append") -and $_.PolicyAssignmentScope -like "*managementGroups*" }
        if ( $StartPolicyComplianceScan ) {
